...
// ---------- public form browse ----------
//
// Endpoints exposed to the public petenak role (same hardcoded token as
// the registration flow). Used by `/pendaftaran/daftar` (list) and
// `/pendaftaran/daftar/[id]` (detail) so anyone can browse the
// submitted forms without an admin login.
//
// Field names mirror the backend envelope exactly (snake_case). They
// do NOT match the local `Peternak` type used by the wizard — keep
// these types separate so the local wizard can evolve independently.
